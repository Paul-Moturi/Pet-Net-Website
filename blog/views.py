...
def profile(request):
    ...
    Pets = Pet.objects.filter(user=user)  # Get pets associated with the user
    user_posts = Post.objects.filter(author=user).order_by('-created_at')  # Get user's posts, newest first
    post_count = user_posts.count()
    try:
        friend_list = FriendList.objects.get(user=request.user).friends.all()
        followers_count = friend_list.count()
    except FriendList.DoesNotExist:
        friend_list = []
        followers_count = 0
    return render(request, 'blog/profile.html', {'user': user, 'Pets': Pets, 'user_posts': user_posts, 'post_count': post_count,
